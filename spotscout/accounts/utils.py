...
def send_code_to_user(email, context="registration"):
    """
    Sends an OTP email based on the context of the action. 
    Context can be 'registration' for user signup or 'ban_delete' for admin ban/delete actions.
    """
    ...
    # Decide the subject and email content based on the context
    ...
    elif context == "ban_delete":
        subject = "Admin Action Verification Required"
        email_body = format_html(
            '''
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px; background-color: #f9f9f9;">
                <h2 style="color: #333; text-align: center;">Action Confirmation: Admin Ban/Delete</h2>
                <p style="color: #555; text-align: center;">Hi <strong>{first_name}</strong>,</p>
                <p style="color: #555; text-align: center;">You are attempting to ban or delete an Admin on <a href="https://{current_site}" style="color: #0066cc; text-decoration: none;">{current_site}</a>. Please confirm this action by entering the following one-time passcode (OTP) or click the link below:</p>
                <div style="font-size: 24px; font-weight: bold; color: #333; text-align: center; margin: 20px 0; border: 1px solid #ccc; border-radius: 5px; padding: 10px; background-color: #fff;">
                    {otp_code}
                </div>
                <p style="color: #555; text-align: center;">If you did not initiate this action, please contact support immediately.</p>
                <p style="color: #555; text-align: center;">Best regards,<br>The Spotscout Team</p>
                <hr style="border-top: 1px solid #eee; margin: 20px 0;">
                <p style="font-size: 12px; color: #999; text-align: center;">This is an automated message, please do not reply.</p>
            </div>
